...
public:
	...
	SC_HAS_PROCESS(CacheModule);

	CacheModule(sc_module_name n, CacheConfiguration config):
		sc_module(n),
		cache(name(), config)
	{
		logger.name = name();
	...
	void handle_request() {
		while(1) {
			...
			if(sc_uint<32>(rdata_cpu) == 0x00000013) {
				std::cout << "HIT STOP" << hex << sc_uint<32>(rdata_cpu) << std::endl;
				sc_stop();
			}
